...
from suomichat.gpt import GPT, GPTConfig, Linear
from suomichat.dataloader import tokenizing_distributed_data_loader_bos_bestfit, tokenizing_distributed_data_loader_with_state_bos_bestfit
from suomichat.common import compute_init, compute_cleanup, print0, DummyWandb, print_banner, get_base_dir, autodetect_device_type, get_peak_flops, COMPUTE_DTYPE, COMPUTE_DTYPE_REASON, is_ddp_initialized
from suomichat.tokenizer import get_tokenizer, get_token_bytes
from suomichat.checkpoint_manager import save_checkpoint, load_checkpoint
...
wandb_run = DummyWandb() if use_dummy_wandb else wandb.init(project="suomichat", name=args.run, config=user_config)

# Flash Attention status
from suomichat.flash_attention import USE_FA3
using_fa3 = USE_FA3
if using_fa3:
...
if args.fp8:
    if device_type != "cuda":
        ...
        from suomichat.fp8 import Float8LinearConfig, convert_to_float8_training
...
if batch_ratio != 1.0:
    # SGD: linear scaling with batch size is standard (not used in suomichat)
