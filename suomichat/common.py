"""
Common utilities for suomichat.
"""
...
# Override with SUOMICHAT_DTYPE env var: "bfloat16", "float16", "float32"
_DTYPE_MAP = {"bfloat16": torch.bfloat16, "float16": torch.float16, "float32": torch.float32}
def _detect_compute_dtype():
    env = os.environ.get("SUOMICHAT_DTYPE")
    if env is not None:
        return _DTYPE_MAP[env], f"set via SUOMICHAT_DTYPE={env}"
    if torch.cuda.is_available():
        ...
        # Users can still force fp16 via SUOMICHAT_DTYPE=float16 if they know what they're doing.
...
def get_base_dir():
    # co-locate suomichat intermediates with other cached data in ~/.cache (by default)
    if os.environ.get("SUOMICHAT_BASE_DIR"):
        suomichat_dir = os.environ.get("SUOMICHAT_BASE_DIR")
    else:
        home_dir = os.path.expanduser("~")
        cache_dir = os.path.join(home_dir, ".cache")
        suomichat_dir = os.path.join(cache_dir, "suomichat")
    os.makedirs(suomichat_dir, exist_ok=True)
    return suomichat_dir
